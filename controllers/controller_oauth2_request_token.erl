...
%% "Request Tokens are only good for obtaining User approval" (Step 1, before user authorization)

-module(controller_oauth2_request_token).
        ...
        true ->
            mod_oauth2:serve_oauth(ReqData, Context, 
                          fun(URL, Params, Consumer, Signature) ->
                                  %Token = m_oauth2_app:secrets_for_verify(none, Consumer, mod_oauth2:oauth_param("oauth_token", ReqData), Context),
                                  ...
                                  case oauth:verify(Signature, atom_to_list(ReqData#wm_reqdata.method), URL,
                                                    Params, mod_oauth2:to_oauth_consumer(Consumer, SigMethod), "") of
                                      true ->
                                          {ok, Token} = m_oauth2_app:request_token(Consumer, Context),

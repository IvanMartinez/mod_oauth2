        ...
        true ->
            mod_oauth2:serve_oauth(ReqData, Context, 
                fun(URL, Params, Consumer, Signature) ->
                        case mod_oauth2:oauth_param("oauth_token", ReqData) of
                            undefined ->
                                mod_oauth2:authenticate("Missing oauth_token.", ReqData, Context);
                            ParamToken ->
                                case m_oauth2_app:secrets_for_verify(request, Consumer, ParamToken, Context) of
                                    undefined ->
                                        mod_oauth2:authenticate("Request token not found.", ReqData, Context);
                                    Token ->
                                        SigMethod = mod_oauth2:oauth_param("oauth_signature_method", ReqData),
                                        case oauth:verify(Signature, atom_to_list(ReqData#wm_reqdata.method), URL,
                                                          Params, mod_oauth2:to_oauth_consumer(Consumer, SigMethod), mod_oauth2:str_value(token_secret, Token)) of
                                            true ->
                                                case m_oauth2_app:exchange_request_for_access(Token, Context) of
                                                    {ok, NewToken} ->
                                                    ...
                                                    {false, Reason} ->
                                                        mod_oauth2:authenticate(Reason, ReqData, Context)
                                                end;
                                            false ->
                                                mod_oauth2:authenticate("Signature verification failed.", ReqData, Context)
                                        end
                                end
                        end
                end

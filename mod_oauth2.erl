            ...
            case serve_oauth(ReqData, Context, 
                fun(URL, Params, Consumer, Signature) ->
                        case oauth_param("oauth_token", ReqData) of
                            ...
                            ParamToken ->
                                case m_oauth2_app:secrets_for_verify(access, Consumer, ParamToken, Context) of
                                    undefined ->
                                        {false, authenticate("Access token not found.", ReqData, Context)};
                                    Token ->
                                        case m_oauth2_app:check_nonce(Consumer, Token, oauth_param("oauth_timestamp", ReqData), oauth_param("oauth_nonce", ReqData), Context) of
                                            {false, Reason} ->
...
serve_oauth(ReqData, Context, Fun) ->
    ...
    case Version of
        "1.0" ->
            ...
            %SigMethod = oauth_param("oauth_signature_method", ReqData),
            case m_oauth2_app:consumer_lookup(ConsumerKey, Context) of
                undefined ->
...
test() ->
    ...
    ?DEBUG(m_oauth2_app:consumer_lookup("Foo", Ctx)),
    ok.
...
is_allowed(Id, Service, Context) ->
    not(z_service:needauth(Service)) orelse
        lists:member(Service, [proplists:get_value(service, S)
                               || S <- m_oauth2_perms:all_services_for(Id, Context)]).
